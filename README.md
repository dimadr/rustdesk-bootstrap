...
Утилита сделана для реальных сценариев удалённой помощи.

Иногда на другой стороне человек, которому невозможно объяснить, где находятся настройки RustDesk, куда вставлять сервер, relay и публичный ключ, и чем это всё отличается.

В этом случае можно заранее собрать небольшой `support.exe` под свой self-hosted RustDesk-сервер и отправить его пользователю.

Пользователь запускает один файл. Ручная настройка не требуется.

EXE можно отправить напрямую или упаковать в архив с паролем — в зависимости от канала доставки и вашего уровня параноидальности.
